...
CREATE VIEW book_vw AS (
    SELECT book.id, book.titre, book.auteur, genre.name AS genre, site.city AS site, book.year AS"années", book.page
    From book 
...
SELECT titre, page, genre FROM book_vw WHERE page > 500 order by page DESC;
...
CONSTRAINT fk_comment_book
    FOREIGN KEY (book_id)
    REFERENCES book(id)
)ENGINE=InnoDB;CREATE VIEW book_vw AS (
    SELECT book.id, book.titre, book.auteur, genre.name AS genre, site.city AS site, book.year AS"années", book.page
    From book 
    INNER JOIN genre ON book.genre_id = genre.id
    INNER JOIN site ON book.site_id = site.id
...
UPDATE book 
SET user_id = 4
WHERE id = 4;CREATE VIEW book_vw AS (
    SELECT book.id, book.titre, book.auteur, genre.name AS genre, site.city AS site, book.year AS"années", book.page
    From book 
    INNER JOIN genre ON book.genre_id = genre.id
    INNER JOIN site ON book.site_id = site.id
...
VALUES (2,2, "c'est de la balle"),
        (4, 3, "j'ai adoré ce livre"),
        (4, 1, "une masteur classe");
...
VALUES (2,2, "c'est de la balle"),
        ...
        CREATE VIEW comment_vw AS(
            SELECT book.titre, user.user_name AS utilisateur, comment.comment_content
            FROM book
